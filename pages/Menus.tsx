  ...
  fixedMenuArray.map((menuArray) => {
    totalMenuInfo.find((infoArray) => infoArray.sandwich_name === menuArray.name)
      ? ((menuArray.recipes = parseInt(
          totalMenuInfo.find((infoArray) => infoArray.sandwich_name === menuArray.name)?.recipe_count ?? "0"
        )),
        (menuArray.favorit = parseInt(
          totalMenuInfo.find((infoArray) => infoArray.sandwich_name === menuArray.name)?.like_count ?? "0"
        )),
        (menuArray.likeRecipe = parseInt(
          totalMenuInfo.find((infoArray) => infoArray.sandwich_name === menuArray.name)?.recipe_like_count ?? "0"
        )))
      : null;
  });
