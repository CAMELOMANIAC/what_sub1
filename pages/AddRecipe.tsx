...
const IngredientsRadarChart = dynamic(
	() => import('../components/IngredientRadarChart'),
	{
...
const AddRecipe = ({param}: {param: string}) => {
	...
	const useCheckInput = () => {
		...
		const onChange = e => {
			e.target.value === ''
				? setArray([])
				: array.includes(e.target.value)
					? setArray(prev =>
							prev.filter(item => item !== e.target.value),
						)
					: setArray(prev => [...prev, e.target.value]);
		};
	...
	const handleClickHander = useCallback(ref => {
		//이벤트를 자식에게 전달해주니까 useCallback으로 함수 메모라이즈
		if (ref.current) {
	...
	const progressBarButtons: progressBarButtonsType[] = [
		{
			...
			text: '레시피 이름',
			ref: recipeNameRef,
			handleClick: () => handleClickHander(recipeNameRef),
		},
		{
			id: 'breadButton',
			text: '빵 선택',
			ref: breadRef,
			handleClick: () => handleClickHander(breadRef),
		},
		{
			id: 'cheeseButton',
			text: '치즈 선택',
			ref: cheeseRef,
			handleClick: () => handleClickHander(cheeseRef),
		},
		{
			id: 'toastingButton',
			text: '토스팅 여부',
			ref: toastingRef,
			handleClick: () => handleClickHander(toastingRef),
		},
		{
			id: 'vegetableButton',
			text: '채소 선택',
			ref: vegetableRef,
			handleClick: () => handleClickHander(vegetableRef),
		},
		{
			id: 'sauceButton',
			text: '소스 선택',
			ref: sauceRef,
			handleClick: () => handleClickHander(sauceRef),
		},
	];
	...
	useEffect(() => {
		...
		const isNotComplete = Object.entries([
			recipeName,
			param,
			bread.state,
			toasting.state,
	...
	const recipeAddHandler = () => {
		if (isComplete) RecipeMutation.mutate();
			...
			<main className={'w-full max-w-screen-lg mx-auto mb-[80px] pt-2'}>
				<div className="w-screen md:w-[1024px] grid grid-cols-2 md:grid-cols-6">
					<div className="hidden md:block md:col-span-3 h-[300px] mt-[10%] sticky top-[10%]">
						...
						<div className="whitespace-pre-line ml-8 text-sm">
							{menuArray[index].summary}
						</div>
						<Image
							width={500}
							height={350}
							src={`/images/sandwich_menu/${param}.png`}
							alt={String(param)}
							className="object-contain object-right h-[350px] drop-shadow-lg"></Image>
				...
				<RecipeNav
					...
					completeHandler={handleClickHander}
