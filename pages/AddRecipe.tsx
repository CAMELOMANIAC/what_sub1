...
const IngredientsRadarChart = dynamic(() => import("../components/IngredientRadarChart"), {
...
const AddRecipe = ({ param }: { param: string }) => {
  ...
  const useCheckInput = () => {
    ...
    const onChange = (e) => {
      e.target.value === ""
        ...
        ? setArray((prev) => prev.filter((item) => item !== e.target.value))
  ...
  useEffect(() => {
    ...
    const isNotComplete = Object.entries([recipeName, param, bread.state, toasting.state]).some(
      ([_key, value]) => value === ""
    );
        ...
        <div className="w-screen md:w-[1024px] grid grid-cols-2 md:grid-cols-6">
          <div className="hidden md:block md:col-span-3 h-[300px] mt-[10%] sticky top-[10%]">
            ...
            <Image
              ...
              alt={String(param)}
              className="object-contain object-right h-[350px] drop-shadow-lg"
            ></Image>
          ...
          <div className={`col-span-3 pt-[10%] p-10 md:p-0 overflow-y-auto min-w-[640px] md:min-w-0`} ref={rootRef}>
            ...
            <button
              className={`rounded-md shadow-sm p-6 w-full ${
                isComplete ? "bg-green-600 text-white" : "bg-white text-gray-300"
              } `}
