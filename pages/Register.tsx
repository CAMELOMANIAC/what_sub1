...
const Register = () => {
	...
	const handleKakaoAccount = () => {
		router.push(
			`https://kauth.kakao.com/oauth/authorize?response_type=code&client_id=${process.env.KAKAO_RESTAPI_KEY}&redirect_uri=${process.env.URL}/api/users/kakao?register=1&client_secret=${process.env.KAKAO_CLIENT_SECRET}`,
		);
	};
					...
					{kakaoCode === '' ? (
						...
						<>
							...
							<section className="w-[300px] text-sm mb-4">
								<p>
									카카오 계정으로 회원 가입시 메일 인증 없이
									회원가입이 가능하며 추후 로그인 시 아이디,
									비밀번호를 입력하지 않고 카카오 계정으로
									즉시 로그인 할 수 있습니다.
								</p>
							</section>
							<section className="p-2">
