...
export const getRecipeTag = async (sandwichMenu: string): Promise<Array<{ tag_table_tag_name: string }> | Error> => {
    ...
    try {
        ...
        const results: Array<{ tag_table_tag_name: string }> | Error = await executeQuery({
            query: query,
            values: [sandwich]
        });
        ...
        if (Array.isArray(results) && results.length < 1) {
            ...
            return results[0].tag_table_tag_name;
        }
