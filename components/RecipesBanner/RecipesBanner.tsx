    ...
    return (
        <>
            {router.isReady && router.query.param && (router.query.param).length !== 0 ? (
                ...
                (searchParams?.has('write') && <WriteRecipesBanner ref={ref}/>) ||
                (searchParams?.has('favorite') && <FavoriteRecipesBanner ref={ref}/>) ||
                <QueryRecipesBanner ref={ref} menuData={menuData} recipeData={recipeData}/>
            )}
