...
const WriteRecipesBanner = ({ref}) => {
    ...
    useEffect(() => {
        ...
        const right = menuLikeArray.slice(menuLikeArray.length / 2, menuLikeArray.length);
        setMenuLikeArrayL(left);
        setMenuLikeArrayR(right);
    }, [menuLikeArray]);

    const recipeLikeCount = useSelector((state: RootState) => state.user.menuLikeArray.length);
    const menuLikeCount = useSelector((state: RootState) => state.user.recipeLikeArray.length);
    const MenuLikeButton = ({menuName}:{menuName:string}) => {
        const {menuLikeHandler} = useMenuLike(menuName);
        const [hover, setHover] = useState<boolean>(false);
        return (
            <button className='flex items-center text-xl text-green-600' onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} onClick={() => menuLikeHandler()}>
                {!hover ? <PiHeartStraightFill className='inline-block' /> : <PiHeartStraight className='inline-block' />}
            </button>
        )
    };
        ...
        <section className={`relative flex justify-center w-screen right-0 bg-white border-gray-200 border-b min-w-[640px]`} ref={ref}>
            ...
            <section className="flex flex-col justify-start pt-4 md:pb-10 w-full max-w-[1024px]">
                <article className='flex flex-col pb-5 pl-4 border-l'>
                    <h1 className='font-bold text-3xl inline text-black pb-4'>{user.userName}님이 남긴 좋아요</h1>
                    <div className='whitespace-pre-line'>
                        <div className='flex flex-row text-sm m-2'>
                            <div className='w-40 px-3'>
                                <div className='text-gray-500'>레시피 좋아요 갯수</div>
                                <div className='font-bold'>{recipeLikeCount}</div>
                            </div>
                            <div className='border-l w-40 px-3'>
                                <div className='text-gray-500'>메뉴 좋아요 갯수</div>
                    ...
                    <article className='md:border-l md:mb-0 px-4 col-span-1 border-l-0'>
                        ...
                        {Array.isArray(menuLikeArrayL) && menuLikeArrayL.map((item, index) => (
                            <div key={index} className='font-normal text-gray-500 grid grid-cols-10 grid-flow-row h-10 my-2'>
                                ...
                                <span className='col-span-2 flex items-center justify-center text-sm text-black'>
                                    <Link href={`/Recipes?param=${item}`}><MdOutlineArrowForward/></Link>
                                </span>
                            </div>
                        ))}
                    </article>
                    <article className='md:border-l md:border-b-0 px-4 col-span-1 border-b border-l-0 md:mb-3'>
                        <span className='invisible font-bold hidden md:block'>메뉴 좋아요</span>
