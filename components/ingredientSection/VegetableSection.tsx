        ...
        <IngredientsSection ref={ref} id='vegetable'>
            ...
            <ul className='mb-4 p-4'>
                {vegetableArray.map((item) => (
                    <InputBox key={item.name} inputType='checkBox' name={item.name} addContext='' checked={prop1.array.includes(item.name)} onChange={prop1.onChange}></InputBox>
                ))}
                <EmptyCheckBox prop={prop1} text='신선 채소 없음'></EmptyCheckBox>
            </ul>
            <h3 className='text-xl font-[seoul-metro]'>절임 채소</h3>
            <ul className='p-4'>
                {pickleArray.map((item) => (
                    <InputBox key={item.name} inputType='checkBox' name={item.name} addContext='' checked={prop2.array.includes(item.name)} onChange={prop2.onChange}></InputBox>
                ))}
                <EmptyCheckBox prop={prop2} text='절임 채소 없음'></EmptyCheckBox>
            </ul>
        </IngredientsSection>
    );
});
