...
import {StyleTag} from '../RecipesBanner/QueryRecipesBanner';
...
const RecipeNameSection = forwardRef<HTMLDivElement, propsType>(
	({prop}, ref) => {
		...
		const addInputHandler = () => {
			if (
				prop.tagArray.every(item => item !== tagInput) &&
				tagInput.replaceAll(' ', '') !== ''
			) {
		...
		const addTagHandler = tag => {
			if (
				prop.tagArray.every(item => item !== tag) &&
				tag.replaceAll(' ', '') !== ''
			) {
		...
		const tagSearch = async ({
			tag,
			param,
		}: {tag: string; param?: never} | {tag?: never; param: string}) => {
			try {
				const result = await fetch(
					tag
						? `/api/recipes/tag?tag=${encodeURIComponent(tag)}`
						: param
							? `/api/recipes/tag?param=${encodeURIComponent(param)}`
							: '',
					{cache: 'force-cache'},
				); //태그는 중요하지 않으므로 브라우저 캐시를 사용
		...
		const trottleRef = useRef(
			throttle((tagInput: string) => {
		...
		return (
			<IngredientsSection ref={ref} id="recipeName">
				...
				<div className="p-2">
					...
					<div className="p-2">
						추천태그:
						{!tagData.find(item => item === tagInput) &&
							tagInput.length > 0 && (
								<StyleTag onClick={addInputHandler}>
									+{tagInput}
								</StyleTag>
							)}
						{tagData &&
