        ...
        <IngredientsSection ref={ref} id='cheese'>
            ...
            <ul className='p-2'>
                {cheeseNutrientArray.map((item) => (
                    <RadioBox key={item.name} name={item.name} addContext='' checked={prop1.state === item.name} onChange={prop1.onChange}></RadioBox>
                ))}
            ...
            <div className='p-2'>
                ...
                <ul className={isShow ? 'max-h-[1000px] transition-all duration-500' : 'max-h-0 overflow-hidden transition-all duration-500'}>
                    ...
                    {cheeseNutrientArray.map((item) => (
                        <RadioBox key={item.name} name={item.name} addContext='' checked={prop2.state === item.name} onChange={prop2.onChange}></RadioBox>
                    ))}
                </ul>
            </div>
        </IngredientsSection>
