        ...
        <IngredientsSection ref={ref} id='cheese'>
            ...
            <ul className='p-2'>
                {cheeseNutrientArray.map((item) => (
                    <InputBox key={item.name} inputType='radio' name={item.name} addContext='' checked={prop1.state === item.name} onChange={prop1.onChange}></InputBox>
                ))}
                <EmptyRadioBox name='치즈' addContext=' 없음' checked={prop1.state === ''} onChange={prop1.onChange} />
            </ul>
            ...
            <div className='p-2'>
                ...
                <ul className={isShow ? 'max-h-[1000px] transition-all duration-500' : 'max-h-0 overflow-hidden transition-all duration-500'}>
                    <EmptyRadioBox name='치즈 추가' addContext=' 없음' checked={prop2.state === ''} onChange={prop2.onChange} setIsShow={setIsShow}/>
                    {cheeseNutrientArray.map((item) => (
                        <InputBox key={item.name} inputType='radio' name={item.name} addContext='' checked={prop2.state === item.name} onChange={prop2.onChange}></InputBox>
                    ))}
                </ul>
            </div>
        </IngredientsSection>
