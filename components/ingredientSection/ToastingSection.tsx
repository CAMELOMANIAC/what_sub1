	...
	return (
		<IngredientsSection ref={ref} id="toasting">
			...
			<div className="w-full h-12 border border-gray-200">
				<button
					onClick={() => prop.setState(true)}
					className={`w-1/2 h-full rounded-l ${prop.state && 'bg-green-600 text-white'}`}>
					예
				</button>
				<button
					onClick={() => prop.setState(false)}
					className={`w-1/2 h-full rounded-r ${prop.state === false && 'bg-green-600 text-white'}`}>
					아니오
				</button>
			</div>
		</IngredientsSection>
