...
const CardModal = ({ recipe, setIsActive }: props) => {
    ...
    return (
        <div className='fixed bg-gray-600/10 top-0 left-0 w-full h-full backdrop-blur-sm z-10'
            ...
            <article className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[640px] md:w-[768px] lg:w-[1024px] bg-white text-black shadow-lg rounded-lg'>
                <button className={'fixed right-5 top-5 z-10'} onClick={() => setIsActive(false)}>
                    <GrClose/>
                </button>
                <div className='grid grid-cols-6 gap-4 md:grid-cols-7'>
                    <CardNav className='h-full rounded-l-lg hidden md:col-span-2 md:flex md:justify-end' setPage={setPage} page={page}></CardNav>
                    <section className='col-span-6 md:col-span-5 p-5'>
                        <div className='mb-5'>
                            <div className='flex flex-row items-center mb-2'>
                                ...
                                <div className='ml-auto mt-auto text-right text-sm text-gray-400'>{recipe.tag}</div>
                            </div>
                            <CardHorizontalNav className='col-span-6 px-10 pb-10 border-b-[1px] rounded-l-lg md:hidden' setPage={setPage} page={page}></CardHorizontalNav>
                        </div>
                        {page === 0 && <SummaryPage recipe={recipe}></SummaryPage>}
                        {page !== 0 && <IngredientsPage recipe={recipe} page={page} className='h-[500px]' />}
                    </section>
                </div>
            </article>
        </div>
