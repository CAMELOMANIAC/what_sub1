...
import {
	...
	cheeseNutrientArray,
	menuNutrientArray,
	ingredientsArray as addIngredientsArray,
	pickleArray,
	sauceNutrientArray,
	vegetableArray,
...
const ImgDiv = styled.div<imgProps>`
	background: linear-gradient(rgba(0, 0, 0, 0) 10%, white 100%),
...
const IngredientsPage = ({recipe, page, className}: props) => {
	...
	const addIngredient = addIngredientsArray
		.filter(item => ingredientsArray.includes(item.name))
		.map(item => item.name);
	const bread = breadNutrientArray.find(item =>
		ingredientsArray.includes(item.name),
	)?.name;
	const breadSummary = breadNutrientArray.find(item =>
		ingredientsArray.includes(item.name),
	)?.summary;
	const cheese = cheeseNutrientArray
		.filter(item => ingredientsArray.includes(item.name))
		.map(item => item.name);
	const cheeseSummary = cheeseNutrientArray
		.filter(item => ingredientsArray.includes(item.name))
		.map(item => item.summary);
	const vegetable = vegetableArray
		.filter(item => ingredientsArray.includes(item.name))
		.map(item => item.name);
	const pickle = pickleArray
		.filter(item => ingredientsArray.includes(item.name))
		.map(item => item.name);
	const sauce = sauceNutrientArray
		.filter(item => ingredientsArray.includes(item.name))
		.map(item => item.name);
	const sauceSummary = sauceNutrientArray
		.filter(item => ingredientsArray.includes(item.name))
		.map(item => item.summary);
			...
			case 1:
				return (
					<div className="flex flex-col w-full h-full pb-10">
						<ImgDiv
							className=""
							$image={'/images/sandwich_menu/' + param + '.png'}>
						...
						{addMeat?.length && (
							<ImgDiv
								className=""
								$image={
									'/images/sandwich_menu/' + addMeat + '.png'
								}>
								<div className="flex flex-col justify-end w-full h-full pl-10">
