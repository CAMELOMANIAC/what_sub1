...
const SearchBar = (props: { className?: string; filterState?: string[] }) => {
  ...
  useEffect(() => {
    ...
    if (typeof query !== "undefined") {
      ...
      if (inputRef.current) inputRef.current.value = String(query);
    }
  }, [router.query]);
  ...
  return (
    <div
      className={`group relative flex flex-wrap items-center focus-within:text-yellow-500 text-green-600 w-[70%] md:w-[50%] mx-auto ${props.className}`}
    >
      <input
        type="search"
        name="serch"
        placeholder="다른 사람은 무엇을 먹을까요?"
