...
const StyledDiv = styled.div`
	background: linear-gradient(
		...
		rgb(234 179 8 / var(--tw-bg-opacity)) 40%,
		rgb(22 163 74 / var(--tw-bg-opacity)) 40%,
		...
		<StyledDiv className="absolute w-screen left-0 top-[48px] mx-auto grid grid-cols-6 bg-white overflow-hidden">
			...
			<div className="col-span-3 h-[300px]">
				<Image
					width={600}
					height={600}
					src={`/images/sandwich_menu/${selected.name}.png`}
					alt={selected.name}
					className="absolute right-[50%] object-contain object-right h-[350px] drop-shadow-lg"></Image>
			</div>
			<div className="col-span-3 whitespace-pre-line flex flex-col justify-center">
				...
				<div className="sm:flex flex-row hidden">
					{selected.ingredients.map(item => (
						<Image
							...
							key={item}
							className="object-cover w-10 aspect-square rounded-md mr-1 mb-8"
							alt="item"></Image>
					))}
				</div>
				<div className="flex sm:flex-row flex-col">
					<Link
						...
						className="font-bold rounded-full px-3 py-2 mr-2 text-black bg-yellow-500 flex justify-center items-center">
						자세히 보기
						<IoIosArrowForward className="inline-block text-xl" />
					</Link>
					{sessionCheck ? (
						<Link
							href={{
							...
							레시피 작성
							<RiPencilFill className="inline-block text-xl" />
						</Link>
					) : (
						<button
