...
const RecipeNav = ({
			...
			entries => {
				entries.forEach(entry => {
					if (entry.isIntersecting) {
						//isIntersecting속성은 감지 영역에 들어온 요소만 가지는 속성
	...
	return (
		<NavBackground className="p-6 hidden md:grid grid-cols-7 grid-rows-1 font-[seoul-metro] ">
			...
			<button
				className="col-span-1 flex justify-center align-middle border-t-[8px] border-green-600 mb-[30%]"
				disabled={!isComplete}
				...
				<div
					className={
						`col-span-1 text-center absolute mt-1 ` +
						(!isComplete && 'text-gray-300')
					}
					onClick={() => completeHandler(completeRef)}>
					작성완료
				</div>
			</button>
		</NavBackground>
