	...
	useEffect(() => {
		...
		Object.entries(context).forEach(([key, value]) => {
			if (
				...
				const nutrientsItem = nutrientsArray.find(
					item =>
						item.name + ' 추가' === value || item.name === value,
				);
				if (nutrientsItem) {
					setCalorie(prev => prev + nutrientsItem.kcal);
					setProtein(prev => prev + nutrientsItem.protein);
					setSaturatedFats(
						prev => prev + nutrientsItem.saturatedFats,
					);
		...
		return (
			<div className="custom-tooltip">
				<p className="label">
					{label}:
					{label === '칼로리' &&
						(payload[0].value * 2000).toFixed(1) +
							'kcal' +
							`(${(payload[0].value * 100).toFixed(1)}%)`}
					{label === '단백질' &&
						(payload[0].value * 50).toFixed(1) +
							'g' +
							`(${(payload[0].value * 100).toFixed(1)}%)`}
					{label === '포화지방' &&
						(payload[0].value * 20).toFixed(1) +
							'g' +
							`(${(payload[0].value * 100).toFixed(1)}%)`}
					{label === '당분' &&
						(payload[0].value * 50).toFixed(1) +
